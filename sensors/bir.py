...
class BIR(object):
    ...
    ButtonsNames = ["LEFT", "RIGHT", "UP", "DOWN", "RED", "GREEN", "WHITE", "OK", "BLUE", "YELLOW", "PURPLE", "LIGHT_BLUE", "8", "9", "*", "OFF","#"]  # String list in same order as HEX list
...
def run_bir_loop(bir, delay, callback, stop_event, settings):
    while True:
        in_data = convertHex(bir.getBinary())  # Runs subs to get incoming hex value
        for button in range(len(bir.Buttons)):  # Runs through every value in list
            if hex(bir.Buttons[button]) == in_data:  # Checks this against incoming
                print(bir.ButtonsNames[button])  # Prints corresponding english name for button
